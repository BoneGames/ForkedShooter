...
public class BehaviourAI : MonoBehaviour
{
    ...
    public Vector3 GetAvoidanceWaypoint(Vector3 _playerPosition)
    {
        ...
        if (closest)
        {
            ...
            Vector3 end = closest.transform.position; // Note(Manny): FIX THIS
    ...
    public void RespawnFromStartPosition()
    {
        // remove ai sense data
        playerTarget = null;
        sMF.targetLastSeen = Vector3.zero;
        sMF.inspectionPoints.Clear();
        // re-run sense tests for new data
        updateAi.Invoke();
        // replenish health and shield
        healthRef.currentHealth = healthRef.maxHealth;
        healthRef.currentShield = healthRef.maxShield;
        // reset position
        transform.position = lastCheckpoint.position;
        Debug.Log("reset enemy position");
    }
    ...
    private void Update()
    {
        ...
        if (hoverTimer <= 1 && startHover)
        {
            ...
            model.localPosition = Vector3.Lerp(new Vector3(model.localPosition.x, startModelHeight, model.localPosition.z),
                                          ...
                                          hoverTimer);
            // move hit collider up with enemy model
